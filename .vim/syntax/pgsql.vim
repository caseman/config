...
" Last Change:  $Date: 2006/04/29 10:51:36 $
" Filenames:    *.pgsql
...
syn keyword pgsqlKeyword	 abort alter aggregate analyze and as
syn keyword pgsqlKeyword	 begin
...
syn keyword pgsqlKeyword	 database domain databases default delete distinct drop declere deallocate
syn keyword pgsqlKeyword	 deferrable deferred defaults do
syn keyword pgsqlKeyword	 explain end exists execute exclusion
syn keyword pgsqlKeyword	 function foreign from full fetch force
syn keyword pgsqlKeyword	 group grant global
syn keyword pgsqlKeyword	 having 
syn keyword pgsqlKeyword	 index into immutable inner initially immediate inherits instead insert in
syn keyword pgsqlKeyword	 join
syn keyword pgsqlKeyword	 key 
syn keyword pgsqlKeyword	 language lock local limit left load
syn keyword pgsqlKeyword	 max min move match
syn keyword pgsqlKeyword	 notify no new null
syn keyword pgsqlKeyword	 or operator outer order old on out
...
syn keyword pgsqlKeyword	 select set strict schema sequence savepoint simple system
syn keyword pgsqlKeyword	 truncate to tranaction trigger table tables temp temporary tablespace
syn keyword pgsqlKeyword	 update unique unlisten
