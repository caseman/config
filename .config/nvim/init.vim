...
" Use locally installed python env to ensure up-to-date pynvim
" use :checkhealth to verify
let g:python3_host_prog="~/.local/bin/python"
...
" Persistent Undo
if has('persistent_undo') && isdirectory(expand('~').'/.local/state/nvim')
  silent !mkdir ~/.local/state/nvim/backups > /dev/null 2>&1
  set undodir=~/.local/state/nvim/backups
  set undofile
  set noswapfile
endif
...
nnoremap <leader>T :bo<space>:split<space>term://bash<CR>
" double-tap esc for terminal normal mode
tnoremap <ESC><ESC> <C-\><C-N>
" simulate insert <C-R>
tnoremap <expr> <C-R> '<C-\><C-N>"'.nr2char(getchar()).'pi'
" same split jumping in terminal
...
" For markdown fold using plugin
autocmd FileType markdown set foldminlines=5 foldmethod=expr foldexpr=StackedMarkdownFolds()
let g:ale_linters = {'markdown': ['mdl']}
let g:markdown_fenced_languages = ['html', 'yaml', 'python', 'bash=sh', 'ocaml', 'c']
...
" Execute shell command async automatically opening the output
command! -nargs=+ Async :copen<BAR>AsyncRun <args>
" Quick rerun last command
nnoremap <leader>! :!!<CR>
